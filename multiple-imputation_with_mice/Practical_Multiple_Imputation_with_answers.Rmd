---
title: "Multiple Imputation in R (answers)"
...
```{r dplyr tibble, results='show', warning = FALSE, message = FALSE}
...
```{r, results='hide',warning = FALSE, message = FALSE, fig.show='show'}
...
-   What is the correlation between bmi and ethnic missingness? how does
...
```{r, misty, results='show',warning = FALSE, message = FALSE}
library(misty)
...
```{r ggmice VIM, results='hide', warning = FALSE, message = FALSE, fig.show='show'}
...
```{r ggcorrplot, results='hide',warning = FALSE, message = FALSE, fig.show='show'}
library(ggcorrplot)
...
```{r, results='show', warning = FALSE, message = FALSE, fig.show='show'}
fit = with(data=imp,expr = lm(ohsdiff~age+sex+bmi+eq5d0+ethnic+imdscore+smoking+ohs0)) #4.2
summary(pool(fit), conf.int = TRUE) #4.3
...
##### 5.1.1. Observe their distribution before (observed) and after (imputed
...
##### 5.1.2. Compare the distributions of observed and imputed data conditional
