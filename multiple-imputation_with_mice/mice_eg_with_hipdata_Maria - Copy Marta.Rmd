...
-   The independent variables (*X's*) are = `age`, `sex`, `bmi`,
    `ethnic`, `imdscore` , `smoking` , `eq5d0`, `occupation`, `ohs0`
...
value (p\<0.05), you can reject the null hypothesis that the data is
"Missing Completely at Random" (MCAR) and you can impute. If p\>0.05,
you can't reject the null hypothesis, therefore, your data might be MCAR
...
There are 228 observations with non-missing values, and there are 227
...
model.matrix(~0+., data=missdata_binary[,-c(exclude)]) %>% 
  cor(use="pairwise.complete.obs")  %>% 
  ggcorrplot(show.diag = F, type="lower", lab=TRUE, lab_size=2)

```

-   Which pairs of variables are have a correlation \> 0.4?

    -   `"height" - "weight"` (*0.47*) and `"bmi" - "weight"` (*0.74*)
        --\> *bmi* is calculated using *height* and *weight*.
    -   `"ohsdiff" - "ohs0"` (*0.62*) and `"ohsdiff" - "ohs6"` (-*0.53*)
        --\> we used *ohs0* and *ohs6* to generate *ohsdiff*
    -   `"ohsdiff" - "eq5d0"` (*0.43*)
    -   `"ohsdiff" - "occupation"` (*0.79*)
    -   `"ohsdiff" - "satisfaction"` (-*0.51*)
    -   `"ohs6" - "satisfaction"` (-*0.51*)
    -   `"ohs6" - "occupation"` (-*0.40*)
    -   `"ohs0" - "eq5d0"` (*0.75*)
    -   `"ohs0" - "occupation"` (*0.51*)
    -   `"age" - "retired"` (*0.69*)
    -   `"sex" - "height"` (+/-*0.64*)
    -   `"sex" - "weight"` (+/-*0.47*)

-   Which variables are very poorly correlated? (e.g., \<\|0.1\|)

    -   `"id"` --\> It is randomly assigned, therefore, expected.
    -   `"smoking"`
    -   `"ethnic"`

-   Think about which variables should be include in the multiple
    imputation as auxiliaries but also which ones should not be
    included.
...
**Option 1:** R has the `interaction.plot()` function built-in. however,
you can only test one linear variable vs two factors. Since bmi and age
are linear, we will categorize bmi to run this function.
...
**Option 2: Fit the two-way ANOVA model or .** Since our bmi is linear
but we had to categorize it to run the plot, we are going to double
check the interaction through and anova.
...
Note that here the interaction
...
```{r mice, results='hide', warning = FALSE, message=FALSE}
library(mice)
